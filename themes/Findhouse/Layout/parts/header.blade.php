				...
				<a href="{{ url(app_get_locale(false,'/')) }}" class="navbar_brand float-left dn-smd" >
					@if((isset($is_homepage) && $is_homepage == true))
						@if($logo_id_tran = setting_item("logo_id_tran"))
							...
							<img class="logo1 " style="width: 70%" src="{{$logo_tran}}" alt="{{setting_item("site_title")}}">
						@endif
					@else
						@if($logo_id = setting_item("logo_id"))
							<?php $logo = get_file_url($logo_id,'full') ?>
							<img class="logo1 " style="width: 70%"  src="{{$logo}}" alt="{{setting_item("site_title")}}">
						@endif
					@endif
		            @if($logo_id = setting_item("logo_id"))
                        <?php $logo = get_file_url($logo_id,'full') ?>
                        <img class="logo2 "  style="width: 70%"src="{{$logo}}" alt="{{setting_item("site_title")}}">
					@endif
				...
				<div class="main_logo_home2 text-center" onclick="window.location.href='{{asset('/')}}'">
					{{-- <a href="{{asset('/')}}" class="navbar_brand float-left dn-smd"> --}}
						@if($logo_id = setting_item("logo_id_mb"))
							<?php $logo = get_file_url($logo_id,'full') ?>
							<img class="nav_logo_img mt20" style="width: 50%" src="{{$logo}}" alt="{{setting_item("site_title")}}">
