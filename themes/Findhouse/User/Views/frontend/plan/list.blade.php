                            ...
                            <div class="inner-box">
                                ...
                                <div class="table-footer">
                                    @if($user and $user_plan = $user->user_plan and $user_plan->plan_id == $plan->id)
                                        @if($user_plan->is_valid)
                                            <div class="d-flex text-center">
                                                ...
                                                @if(setting_item_with_lang('enable_multi_user_plans'))
                                                    <a href="{{route('user.plan.buy',['id'=>$plan->id])}}" class="btn btn-thm">{{__('Repurchase')}}</a>
                                                @endif
                                            </div>
                                        @else
                                            <a href="{{route('user.plan.buy',['id'=>$plan->id])}}" class="btn btn-thm">{{__('Repurchase')}}</a>
                                        @endif
                                    @else
                                        <a href="{{route('user.plan.buy',['id'=>$plan->id])}}" class="btn btn-thm">{{__('Select')}}</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="inner-box">
                                ...
                                <div class="table-footer">
                                    @if($user and $user_plan = $user->user_plan and $user_plan->plan_id == $plan->id)
                                        ...
                                        @else
                                            <a href="{{route('user.plan.buy',['id'=>$plan->id,'annual'=>1])}}" class="btn btn-thm">{{__('Repurchase')}}</a>
                                        @endif
                                    @else
                                        <a href="{{route('user.plan.buy',['id'=>$plan->id,'annual'=>1])}}" class="btn btn-thm">{{__('Select')}}</a>
                                    @endif
                                </div>
                            </div>
                        </div>
