...
jQuery(function ($) {
        ...
        $.ajax({
            ...
            success:function (json) {
                ...
                if(json.status)
                {
                    ...
                    window.history.pushState({}, '', bc_current_url+'?'+$('.bravo_form_search_map').serialize());
    ...
    $(document).on('click', '.filter-item .dropdown-menu', function (e) {
        ...
        .on('click','.mbp_pagination a',function (e) {
            e.preventDefault();
            $('.bravo_form_search_map [name=page]').val($(this).text());
            reloadForm();
        })
    ;

    window.setFieldAndReload = function(name,field){
        console.log(field)
        $('.bravo_form_search_map [name='+name+']').val(field);
        reloadForm();
    }
    window.setTypeParam = function(field,btn){
        const name = 'type';
        $('.bravo_form_search_map [name='+name+']').val(field);
        $(btn).addClass('active').parent().siblings().find('.hvr-text-thm').removeClass('active');
        reloadForm();
    }
